...
public class Part2 {
    ...
    public static void main(String[] args) {
        try {
            URL resource = Part2.class.getClassLoader().getResource("day12/input1.txt");
            ...
            lines.forEach(Part2::createCave);
            lines.forEach(Part2::createConnections);
    ...
    private static boolean canVisit(String conn, List<String> visited) {
        ...
        if (conn.matches("^[A-Z]+$") || !visited.contains(conn)) {
            return true;
        }

        List<String> smallsVisited = visited.stream().filter(cave -> cave.matches("^[a-z]+$")).collect(Collectors.toList());
        List<String> distinctSmallsVisited = smallsVisited.stream().distinct().collect(Collectors.toList());
        return (smallsVisited.size() == distinctSmallsVisited.size());
    }
